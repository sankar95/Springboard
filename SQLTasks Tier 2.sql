...
SELECT DISTINCT CONCAT( firstname,  " ", surname,  "-", Facilities.name ) AS  "Member name - Facility"
FROM Members
...
SELECT Facilities.name as 'Facility Name', CONCAT(surname," ", firstname) 'Facility Used by', CASE WHEN Members.memid > 0 THEN membercost ELSE guestcost END AS "Cost"
FROM Bookings
INNER JOIN Facilities ON Bookings.Facid = Facilities.Facid
